<template>
    <div class="p-4 m-4 flex flex-col justify-center items-center bg-gray-100 ">
      <h1 class="text-3xl font-bold mb-4">Latter-day Saints Resources</h1>
      <hr>
      <div class="flex space-x-4">
        <a href="https://www.churchofjesuschrist.org/" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline mx-4">
          Official Church Website
        </a>
        <a href="https://www.comeuntochrist.org/" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline mx-4">
          ComeUntoChrist.org
        </a>
        <a href="https://www.mormon.org/" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline mx-4">
          Mormon.org
        </a>
        <a href="https://www.lds.org/" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline mx-4">
          LDS.org
        </a>
      </div>
      <hr>
    </div>
  </template>
  
  <script scoped>
  export default {
    name: "LDSLinksPage"
  };
  </script>
  
  <style scoped>
  /* Add any additional styling here */
  </style>
  